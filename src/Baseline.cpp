...
// 以基准 TTTD 参数为 1 倍，按 scale 放大“大块平均尺寸”：
...
// 当前生效的合成式参数（由主菜单选择大块尺寸后设置）。
AmalgamationConfig gAmalgamationConfig = makeAmalgamationConfig(1);
...
struct RunStats {
    ...
    std::size_t amBig;
    std::size_t amDupBig;
    std::size_t amSmallEmitted;
};

// 快照统计，创建一个 RunStats 对象并返回。
...
// 打印单个文件的增量与累计 DER，便于按备份版本定位 2.4 的效果。
auto reportFileDelta(const std::filesystem::path& path,
    ...
    if (mode != Mode::Baseline) {
        std::cout << " small+=" << (after.amSmall - before.amSmall)
                  ...
                  << " smallEmit+=" << (after.amSmallEmitted - before.amSmallEmitted);
    }
...
// 遍历目录下所有普通文件逐个分块。
// recursive=false：只处理顶层文件（DataSet_1 / DataSet_3 / DataSet_4）。
...
auto processDirectory(const std::filesystem::path& dir,
    ...
    // 文件较少时（如 DataSet_1/3/4）逐文件报告，便于定位每个备份的效果。
...
auto resolver(const std::filesystem::path& target, // 目标路径。
              ...
              const Mode mode)                     // Baseline 还是 Amalgamation？
              -> void {

    namespace fs = std::filesystem;

    std::error_code ec;
    if (fs::is_regular_file(target, ec)) {                        // 普通文件？直接处理。
        const RunStats before = snapshotStats();
        processFile(target, mode);
        reportFileDelta(target, mode, before, snapshotStats());
    } else if (fs::is_directory(target, ec)) {                    // 文件夹目录？遍历处理。
...
//   4：合成式（论文 2.4）跑 DataSet_3（未压缩 tar 备份流）。
//   6：合成式（论文 2.4）跑 DataSet_4（合成集中变更备份流）。
...
int main() {
    ...
    // 用于处理目标数据集的递归遍历与否、采用 Baseline 还是 Amalgamation。
    switch (choice) {
        case 1:
            target = kDataSet1;
            recursive = false;
    ...
    if (mode != Mode::Baseline) {
        std::cout << "smallChunks=" << gAmSmallChunks
                  ...
                  << " queries=" << gAmQueryCount
                  << " emittedSmalls=" << gAmSmallEmitted
                  << '\n';
    }
}
