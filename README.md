...
2. 在窗口 `buf[0..2k-1]` 上做**前向搜索** `pos = 0..k`，找第一个重复的大块（恰好 `k` 个连续小块）：
   - 命中 → 先把 `pos` 个前导小块按小块发射，再把该大块按大块发射（`isPrevDupBig = true`）；
   - 未命中但刚离开重复区 → 发射 `k` 个小块作为 transition（`isPrevDupBig = false`）；
...
| 2.4 合成式（大块 ≈ 1k，k=4） | 7.832 | 279914 | ≈ 959 B |
| 2.4 合成式（大块 ≈ 16k，k=64） | 7.359 | 59198 | ≈ 4535 B |

- 在 DER 相近（≈7.4–7.8）的区间内，2.4 把平均块长从 baseline 的 ~1k 提高到 ~4.5k，方向与论文"相近 DER 下更大平均块长"一致。
